...
export default function EventsImgDetails({limit}: {limit?:number}) {
  ...
  if (!limitedEvents.length) return null;
   const className = limit && limit > 0 ? "w-[100%]" : "w-[45%]";
 
  return (
     
     <div className={`${className} space-y-6 flex flex-wrap gap-5 `} >
      {limitedEvents.map((event, i) => (
        <div key={event._id || i} className="rounded-lg overflow-hidden text-white relative">
          ...
          <div className="absolute top-0 left-0 w-full h-full p-4 flex flex-col justify-evenly">
            ...
            <div>
              <div
                className="text-sm px-2 py-1 inline-block rounded"
                style={{ backgroundColor: "#1C7A80" }}
              >
                {event.eventType}
              </div>
